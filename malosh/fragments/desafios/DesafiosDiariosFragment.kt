...
class DesafiosDiariosFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (inicioTemporizador > 0L) {
            // Si el temporizador ya está en progreso, reanudarlo
    ...
    private fun actualizarCheckBoxes(tiempoRestante: Long) {
        ...
        if (porcentajeRestante >= 25) inicioCheckBox.isEnabled = true
        if (porcentajeRestante >= 50) enProgresoCheckBox.isEnabled = true
        if (porcentajeRestante >= 75) casiPorTerminarCheckBox.isEnabled = true
        if (porcentajeRestante >= 100) completadoCheckBox.isEnabled = true
    }
        ...
        val runnable = object : Runnable {
            override fun run() {
                if (tiempoRestante > 0) {
                    ...
                    generarDesafios(registeredHabits) // Generar y mostrar un nuevo desafío
                    mostrarDesafio()
    ...
    private fun setCheckBoxesVisibility(visibility: Int) {
        inicioCheckBox.visibility = visibility
        enProgresoCheckBox.visibility = visibility
        casiPorTerminarCheckBox.visibility = visibility
        completadoCheckBox.visibility = visibility
    }
